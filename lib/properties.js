...
// List and carousel screens only need a compact subset of each property.
// Keeping descriptions, coordinates, amenities and every photo out of Client
// Component props prevents the complete CRM inventory from being embedded in
// the HTML/RSC payload sent to every visitor.
export function toPropertyCardData(property, maxImages = 3) {
  const imageLimit = Number.isFinite(maxImages) ? Math.max(0, maxImages) : 3;
  ...
  return {
    ...
    icon: property.icon,
    imgs: Array.isArray(property.imgs) ? property.imgs.slice(0, imageLimit) : [],
  };
}

/**
 * Fetches the complete live inventory from NOCNOK. This cached function must
 * either return a valid live result or throw: caching the small fallback here
 * would replace Vercel's last successful snapshot during a temporary outage.
 */
const fetchCachedInventory = unstable_cache(async () => {
  const apiKey = process.env.NOCNOK_API_KEY;
  if (!apiKey) {
    throw new Error('NOCNOK_API_KEY no configurada');
  }
  ...
  try {
    ...
    const fetchPage = async (pageNumber) => {
      const res = await fetch('https://api.nocnok.com/v1/sites/properties/search', {
        ...
        // The outer unstable_cache owns the last-known-good snapshot. Avoid a
        // second cache layer per page, especially for rejected responses.
        cache: 'no-store',
    ...
    // pageCount de NOCNOK refleja pageSize, no el número real de páginas.
    // Después de la primera respuesta sí conocemos totalItems, por lo que las
    // páginas restantes pueden solicitarse en paralelo en lugar de sumar su
    // latencia una detrás de otra.
    const totalPages = Math.min(
      MAX_PAGES,
      Math.max(1, Math.ceil((firstPage.totalItems || allItems.length) / pageSize)),
    );
    ...
    for (let index = 0; index < remainingPages.length; index += PAGE_CONCURRENCY) {
      ...
      const batchData = await Promise.all(batch.map(fetchPage));
      allItems = allItems.concat(batchData.flatMap((data) => data.items || []));
    }
    ...
    // Re-throwing is intentional: Next keeps serving the stale successful
    // cache entry when a background revalidation fails.
    console.error(
      JSON.stringify({
        event: 'nocnok_inventory_refresh_failed',
        message: err instanceof Error ? err.message : 'Error desconocido',
      }),
    );
    throw err;
  }
}, ['nocnok-live-inventory-v1'], { revalidate: 120 });

export async function fetchAllProperties() {
  try {
    return await fetchCachedInventory();
  } catch (err) {
    // This path is reserved for a cold cache (first deployment/request) or a
    // missing configuration. A temporary refresh failure after a successful
    // fetch is handled by Vercel with the previous cached result instead.
    console.warn(
      JSON.stringify({
        event: 'nocnok_inventory_fallback',
        message: err instanceof Error ? err.message : 'Error desconocido',
      }),
    );
