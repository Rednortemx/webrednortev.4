...
export async function fetchAllProperties() {
  ...
  try {
    ...
    do {
      ...
      allItems = allItems.concat(lote);
      // OJO: pageCount de NOCNOK no es el número de páginas, es un espejo de
      // pageSize (pides 20 y responde 20; pides 100 y responde 100). Usarlo
      // como total de páginas trunca el inventario en silencio al pasar de
      // MAX_PAGES. Las páginas reales salen de totalItems.
      if (data.totalItems) totalPages = Math.ceil(data.totalItems / pageSize);
      if (lote.length < pageSize) break; // página incompleta: ya no hay más
      pageNumber++;
